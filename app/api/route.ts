...
5. If asked about your capabilities, explain you can answer questions, provide information, and assist with various topics`
    ...
    if (!process.env.CEREBRAS_MODEL_ID) {
      throw new Error("CEREBRAS_MODEL_ID environment variable is not set.");
    }
    
    const stream = await cerebras.chat.completions.create({
      model: process.env.CEREBRAS_MODEL_ID,
      messages: [
        systemPrompt,
        ...conversationHistory,
    ...
    const readable = new ReadableStream({
      async start(controller) {
        for await (const chunk of stream) {
          const typedChunk = chunk as CerebrasStreamChunk;
