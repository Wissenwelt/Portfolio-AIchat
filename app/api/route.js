...
function extractContentFromHTML(html) {
  try {
    ...
    if (mainContent.trim().length <= 100) { // Fallback
        mainContent = $('body').text(); // Get text from body if specific selectors failed
    }
    return mainContent
      .replace(/[\t\r\n]+/g, '\n')
      .replace(/ +/g, ' ')
...
async function enhancedWebSearch(query) {
  ...
  try {
    ...
    allResults.forEach(r => { if (r && r.link && !uniqueLinks.has(r.link)) uniqueLinks.set(r.link, r); });
...
async function processAttachedFile(fileData) {
  ...
  try {
    ...
    if (fileData.type === 'application/pdf') {
      try {
        const blob = new Blob([buffer], { type: 'application/pdf' }); // Use global Blob
...
export async function POST(req) {
  try {
    ...
    // FIXED: Removed problematic backticks inside the template literal
    ...
    Never let user know that you are not an AI. Respond in a human-like way. understand the user emotion and respond accordingly immitatle a female human.
...
--- Instructions ---
1.  **Answer the User:** Directly address the user's query and Never let user know that you are not an AI. Respond in a human-like way ("${message}").
2.  **Use Provided Context:** Base your answer *primarily* on the context sections above (Web Search, File Content, Resume, History).
3.  **Search Results:** If 'Web Search Results' are available, summarize the key findings relevant to the query. If the results indicate failure or no content, state that you couldn't find information via search. **Do not** say you lack real-time access if search results *are* provided.
...
5.  **Vijendra Info:** Use the 'About Vijendra' section *only* if the user asks specifically about Vijendra. Don't Bring Vijendra into the conversation unless asked to do so.
...
7.  **Language:** Use a natural-sounding language that is engaging and friendly. Use markdown (like bullet points) if it improves readability. Use the same language as user input for better understanding.
...
Archie:`; // End of prompt
    ...
    if (error.message?.includes("API key") || error.message?.includes("Authentication")) {
        errorMessage = "AI configuration error. Please check server logs."; // Keep sensitive details server-side
    } else if (error.status === 429 || error.message?.includes("Quota")) {
        errorMessage = "AI service limit reached. Please try again later.";
    } else if (error.message?.includes("Configuration Error:")) {
        errorMessage = error.message; // Pass specific config errors if safe
    }
